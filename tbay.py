...
#  beyonce = User(username="bknowles", password="crazyinlove") ### short form
#  session.add(beyonce)
#  session.commit()
...
#
#  Return the item id and description for all baseballs which were created in the past.
#  Remember to import the datetime object: from datetime import datetime
#  session.query(Item.id, Item.description).filter(Item.name == "baseball", 
#                                                  Item.start_time < datetime.utcnow()).all()
